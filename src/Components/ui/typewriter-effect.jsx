...
export const TypewriterEffect = ({ words, className, cursorClassName }) => {
        ...
        {wordsArray.map((word, idx) => {
          return (
            <div key={`word-${idx}`} className="inline-block">
              ...
              <span>&nbsp;</span> {/* ensures a space between words */}
            </div>
          );
        })}
      </motion.div>
  ...
  return (
    <div
      className={cn(
        "montserrat-strong text-center text-base tracking-tighter max-lg:text-3xl lg:text-5xl",
        className,
      )}
    >
      {renderWords()}
      <motion.span
        ...
        className={cn(
          "inline-block h-4 w-1 rounded-sm bg-blue-500 md:h-6 lg:h-10 max-lg:pb-9",
          cursorClassName,
        )}
      ></motion.span>
    </div>
